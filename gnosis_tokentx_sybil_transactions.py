...
groups_df = pd.read_csv(r"D:\sybil\gnosis_tokentx_large_connected_groups.csv")
            ...
            if tx["from"] in addresses and tx["to"] in addresses and tx["from"] != tx["to"]:
                if tx["input"] == "0x":
                    transfer_str = (f'hash: "{tx["hash"]}" from: "{tx["from"]}" to: "{tx["to"]}" '
                                    f'tokenSymbol: "XDAI" value: "{int(tx["value"]) / (10 ** 18)}"')
...
# save
output_file_path = r"D:\sybil\gnosis_tokentx_token_transfers_per_group.csv"
