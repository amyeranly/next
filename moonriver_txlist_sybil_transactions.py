...
groups_df = pd.read_csv(r"D:\sybil\moonriver_txlist_large_connected_groups.csv")
...
for index, group in enumerate(groups, start=1):
    ...
    for address in addresses:
        transactions = get_transactions_moonbeam(address, API_KEYS_MOONRIVER[0])
            ...
            if tx["from"] in addresses and tx["to"] in addresses and tx["from"] != tx["to"]:
                if tx["input"] == "0x":
                    transfer_str = (f'hash: "{tx["hash"]}" from: "{tx["from"]}" to: "{tx["to"]}" '
                                    f'tokenSymbol: "MOVR" value: "{int(tx["value"]) / (10 ** 18)}"')
...
# save
output_file_path = r"D:\sybil\moonriver_txlist_token_transfers_per_group.csv"
