...
def get_transactions_optimism(address, api_key):
    ...
    for _ in range(3):  # Try up to 3 times
        try:
            response = requests.get(OPTIMISMSCAN_API_URL, params=params, timeout=10)  # Added a timeout for the request
            time.sleep(0.2)
            break
...
df_related_addresses.to_csv(r"D:\sybil\optimism_txlist_related_address_pairs.csv", index=False)

print("Related address pairs on Optimism have been saved to optimism_txlist_related_address_pairs.csv.")
